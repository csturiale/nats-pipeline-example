...
var streamName = strings.ToUpper(string(pipeline.Git))
...
func addConsumer(s int, stream jetstream.Stream, newJS jetstream.JetStream) {
	fmt.Println(fmt.Sprintf("[GIT] # adding consumer-%d", s))
	...
	_, _ = cons.Consume(func(msg jetstream.Msg) {
		...
		err = msg.Ack()
		if _, err := f.WriteString(fmt.Sprintf("%v - %s - [GIT] - acking message", time.Now().Format("2006-01-02 15:04:05.000"), step.PipelineName)); err != nil {
			log.Println(err)
		}
		if err != nil {
			if _, err := f.WriteString(fmt.Sprintf("%v - %s - [GIT] - err acking msg %s\n", time.Now().Format("2006-01-02 15:04:05.000"), step.PipelineName, err.Error())); err != nil {
				log.Println(err)
			}
		}
	})
	fmt.Println(fmt.Sprintf("[GIT] # created consumer-%d", s), cons.CachedInfo().Name)
}
...
func doTheJob(consumerNumber int, s pipeline.Step) {
	...
	defer f.Close()
	if _, err := f.WriteString(fmt.Sprintf("%v - %s - [GIT] consumer-%d Sleeping for %d seconds...\n", time.Now().Format("2006-01-02 15:04:05.000"), s.PipelineName, consumerNumber, randNum)); err != nil {
		log.Println(err)
	}
	fmt.Println(fmt.Sprintf("%v - [GIT] %s consumer-%d Sleeping for %d seconds...", time.Now().Format("2006-01-02 15:04:05.000"), s.PipelineName, consumerNumber, randNum))
	...
	defer f.Close()
	if _, err := f.WriteString(fmt.Sprintf("[GIT]\n")); err != nil {
		log.Println(err)
	}
