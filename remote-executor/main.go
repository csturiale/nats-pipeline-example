...
var streamName = strings.ToUpper(string(pipeline.RemoteExecutor))
...
func addConsumer(s int, stream jetstream.Stream, newJS jetstream.JetStream) {
	fmt.Println(fmt.Sprintf("[REMOTE-EXECUTOR] # adding consumer-%d", s))
	...
	_, _ = cons.Consume(func(msg jetstream.Msg) {
		...
		err = msg.Ack()
		if _, err := f.WriteString(fmt.Sprintf("%v - %s - [REMOTE-EXECUTOR] - acking message", time.Now().Format("2006-01-02 15:04:05.000"), step.PipelineName)); err != nil {
			log.Println(err)
		}
		if err != nil {
			if _, err := f.WriteString(fmt.Sprintf("%v - %s - [REMOTE-EXECUTOR] - err acking msg %s\n", time.Now().Format("2006-01-02 15:04:05.000"), step.PipelineName, err.Error())); err != nil {
				log.Println(err)
			}
		}
	})
	fmt.Println(fmt.Sprintf("%v - [REMOTE-EXECUTOR] # created consumer-%d", time.Now().Format("2006-01-02 15:04:05.000"), s), cons.CachedInfo().Name)
}
...
func doTheJob(consumerNumber int, s pipeline.Step) {
	...
	defer f.Close()
	if _, err := f.WriteString(fmt.Sprintf("%v - %s - [REMOTE-EXECUTOR] consumer-%d Sleeping for %d seconds...\n", time.Now().Format("2006-01-02 15:04:05.000"), s.PipelineName, consumerNumber, randNum)); err != nil {
		log.Println(err)
	}
	fmt.Println(fmt.Sprintf("%v - [REMOTE-EXECUTOR] %s consumer-%d Sleeping for %d seconds...", time.Now().Format("2006-01-02 15:04:05.000"), s.PipelineName, consumerNumber, randNum))
	...
	defer f.Close()
	if _, err := f.WriteString(fmt.Sprintf("[REMOTE-EXECUTOR]\n")); err != nil {
		log.Println(err)
	}
	...
	defer f.Close()
	if _, err := f.WriteString(fmt.Sprintf("%v - [REMOTE-EXECUTOR]\n", time.Now().Format("2006-01-02 15:04:05.000"))); err != nil {
		log.Println(err)
	}
}
